	...
	else
	{
		...
		else
		{
			...
			while(salida!=1)
			{
				...
				cout<<"Introduzca una hora entre las 9 y las 19, y recuerde que las citas empiezan a en punto y duran 15 minutos"<<endl;
				cout<<"hora -> ";
				cin>>hor;
				cout<<"minuto -> ";
				cin>>minus;
				if(atoi(hor)>=9 && atoi(hor)<=19)
				{
					if(atoi(minus)==0 || atoi(minus)==15 || atoi(minus)==30 ||atoi (minus)==45)
					{
						salida=1;
					}
				}
