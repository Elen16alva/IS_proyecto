...
list <string> Historial::cargar_lista_pacientes()
{
	...
	else
	{
		...
		cout<<"*************************************"<<endl;
		cout<<"\nPulse cualquier número para continuar"<<endl;
		char n;
		cin>>n;
	}
	return aux;
...
void Historial::consultar_historial()
{
	...
	if(!lista_pacientesH_.empty())
	{
		...
		fichero.close();
		char c;
		cout<<"Pulse cualquier numero para continuar"<<endl;
		cin>>c;
	}
}
...
void Historial::add_historial()
{
	...
	if(!lista_pacientesH_.empty())
	{
		...
		cout<<"La fecha debe tener este formato 00/00/0000"<<endl;
		cin.ignore();
